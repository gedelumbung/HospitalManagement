...
class laporan_data_pasien extends CI_Controller {
   ...
   public function index($uri=0)
   {
		...
		{
			$d['data_retrieve'] = $this->app_global_admin_model->generate_index_laporan_pasien($GLOBALS['site_limit_medium'],$uri);
   ...
   public function cetak()
   {
		if($this->session->userdata("logged_in")!=""  && $this->session->userdata("level")=="admin")
		{
			$d['data_retrieve'] = $this->app_global_admin_model->generate_index_laporan_pasien_cetak();
			
 			$this->load->view("laporan_data_pasien/bg_cetak",$d);
		}
		else
		{
			redirect(base_url());
   ...
   public function cetak_detail($id_param)
   {
		if($this->session->userdata("logged_in")!=""  && $this->session->userdata("level")=="admin")
		{
			$where['id_pasien'] = $id_param;
			...
			$d['tipe'] = "edit";
 			...
 			$this->load->view("laporan_data_pasien/bg_cetak_detail",$d);
		}
		else
		{
			redirect(base_url());
